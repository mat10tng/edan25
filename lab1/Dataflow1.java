...
class Task implements Runnable {
	...
	public Task(LinkedList workList){
...
class Dataflow {
	...
	public static void liveness(Vertex vertex[], int nthread)
	{
		...
		LinkedList<LinkedList<Vertex>> workLists;
		long			begin;
		long			end;
		...
		workLists = new LinkedList<LinkedList<Vertex>>();
		for( i = 0; i < nthread;  i++){
				workLists.add(new LinkedList<Vertex>());
		}

		for (i = 0; i < vertex.length; ++i) {
			workLists.get(i%nthread).addLast(vertex[i]);
			vertex[i].listed = true;
		}
		Thread threads[] = new Thread[nthread];
		for(i = 0; i< nthread; i ++) {
			Thread t = new Thread(new Task(workLists.get(i)));
			threads[i] = t;
			t.start();
		}
		for( i = 0; i < nthread; i++){
